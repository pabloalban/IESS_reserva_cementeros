...
for (j in c(1:nrow(lista_ben))) {
  ...
  aux <- lista_ben %>%
    ...
    select( ultimo_sueldo_nominal)
  
  aux <- formatC(aux$ultimo_sueldo_nominal,decimal.mark = ",",big.mark = ".", format='f',digits = 2)
  
  
  write.table(aux, file = paste0( parametros$resultado_tablas, 'iess_ultimo_sueldo_',j,'.tex' ),
              sep = ",",
              col.names = FALSE,
              row.names = FALSE,
              quote = FALSE,
